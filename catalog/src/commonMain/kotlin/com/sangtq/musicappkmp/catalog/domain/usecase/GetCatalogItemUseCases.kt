...
class GetArtistUseCase(private val repository: CatalogRepository) {
    /** Artist offline-first (cache + refresh) — xem networkBoundResource. */
    operator fun invoke(id: Long): Flow<Resource<Artist>> = repository.observeArtist(id)
}

class GetPlaylistUseCase(private val repository: CatalogRepository) {
    /** Playlist offline-first (cache + refresh) — xem networkBoundResource. */
    operator fun invoke(id: Long): Flow<Resource<Playlist>> = repository.observePlaylist(id)
}
